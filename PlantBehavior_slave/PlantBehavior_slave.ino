...
unsigned long BreakinteruptTime;
Servo ydersteServo;//top eyelid Servo
int posYdre = 105;//start position eyelid
...
// Start the I2C Bus as Slave on address 9
  ...
  interuptTime= millis()+random(3000,5000);
  BreakinteruptTime= millis()+random(3000,5000);
}
void receiveEvent(int bytes) {
  isEyeActive = Wire.read();    // read one character from the I2C
}
...
{
  shouldClose = true;
  BreakinteruptTime =millis()+random(10000,15000);
  ...
  while(isEyeActive == 1)
  {
    if ((BreakinteruptTime) <= millis()&& eyeClosed ==false)
      {//lukke øje
         for (posYdre =99; posYdre <= 160; posYdre += 1)
        {
          ydersteServo.write(posYdre);// tell servo to go to position in variable 'pos'
         ...
         eyeClosed = true;
         BreakinteruptTime =millis()+random(1000,2000);
      }

      else if ((BreakinteruptTime) <= millis()&& eyeClosed ==true)
      {// åbner øje
       for (posYdre = 160; posYdre >= 99; posYdre -= 1)
          {
            ydersteServo.write(posYdre); // tell servo to go to position in variable 'pos'
